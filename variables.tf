...
variable "namespace" {
  type        = string
  default     = "grafana"
  description = "The K8s namespace in which the grafana service account has been created"
}

variable "settings" {
  type        = map(any)
  default     = {}
  description = "Additional helm sets which will be passed to the Helm chart values, see https://hub.helm.sh/charts/stable/grafana"
}

variable "values" {
  type        = string
  default     = ""
  description = "Additional yaml encoded values which will be passed to the Helm chart, see https://hub.helm.sh/charts/stable/grafana"
}
